...
class MongoDBUtil(object):
    def __init__(self, mongo_url, db_name, collection_name, auto_connect=False):
        ...
        :param auto_connect: default do not connect for multiprocessing
        ...
        self.collection = MongoClient(mongo_url,
    ...
    def __repr__(self):
        return '%s (db_name:%s, collection_name:%s, auto_connect:%s)' % \
               (StringUtil.mask_passwd_in_url(self.mongo_url),
                self.db_name, self.collection_name, self.auto_connect)
    ...
    def find(self, query=None, sort=None, limit=0):
        ...
        for row in self.collection.find(query,
    ...
    def find_one(self, query: dict, limit=0) -> dict:
        if limit > 0:
            return self.collection.find_one(
                query, no_cursor_timeout=True).limit(limit)
        else:
            return self.collection.find_one(
    ...
    def create_index(self, field_list=None, unique=False):
        ...
        for field in field_list:
            self.collection.create_index(
                [(field, ASCENDING)], background=True, unique=unique)
        return

    def insert(self, row: dict):
        return self.collection.insert_one(row)

    def update_one(self, where_query: dict,
                   update_content: dict,
                   upsert=False):
    ...
    def update(self, where_query: dict,
               update_content: dict,
               upsert=False):
...
if __name__ == '__main__':
    MONGO_URL = r'mongodb://%s:%s@%s:%s/%s?authMechanism=MONGODB-CR' % \
                ('user', 'pwd', 'host', '27017', 'db')
    mongo = MongoDBUtil(MONGO_URL, 'db', 'collection')
    mongo.create_index(['title', 'content'])

    # query =
    # "{'url': 'http://news.mk.co.kr/newsRead.php?sc=30000016&year=2016&no=603924'}"
    query = [{"title": ""}, {"content": ""}]
